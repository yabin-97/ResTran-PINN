...
class PINN(nn.Module):
    ...
    def Net_f(self,x, y, z, t):
        ...
        f = u_xt - self.beta*u_x**2 - self.alpha*u_xxxx - self.beta*u*u_xx + self.r*(u_yy + u_zz)
...
if __name__ == "__main__":
    ...
    U0 = u0[idx0, :]
    #add noise
    U0 = U0 + 0.1*np.std(U0)*np.random.randn(U0.shape[0], U0.shape[1])
    ...
    def closure():
        ...
        loss = loss0 + lossb + torch.sin(lossf)
    ...
    models.load_state_dict(torch.load('model_parameters_i.pth', map_location='cpu'))

    # torch.save(models.state_dict(), 'model_parameters_i.pth')
